...
export const config = {
  ...
  deviceId: env.DEVICE_ID ?? "demo-device",
  ...
  repeatersConfigPath: env.REPEATERS_CONFIG_PATH ?? "edge/config/repeaters.json",
  companion: {
    connection: (env.COMPANION_CONNECTION ?? "tcp") as "tcp" | "serial",
    host: env.COMPANION_TCP_HOST ?? "192.168.88.90",
    port: Number(env.COMPANION_TCP_PORT ?? 5555),
  ...
  auth: {
    ...
    useDeviceSigning: env.USE_DEVICE_SIGNING === "true",
  },
};
